...
export function WordSearchExercice() {
			...
			for (const placement of wordPlacements) {
				...
				const forwardMatch =
					placement.positions.every((pos, i) => {
						return (
							currentSelection[i] &&
							currentSelection[i].row === pos.row &&
							currentSelection[i].col === pos.col
						);
	...
	return (
      ...
      <div className="flex gap-2">
        ...
        <Card className="p-4 md:p-6 flex-3">
          <div
          ...
          >
			...
			{isComplete && (
				<Card className="p-6 md:p-8 bg-primary text-primary-foreground text-center animate-fade-in absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-lg z-10">
					<div className="space-y-4">
						<div className="flex justify-center">
							<Trophy className="w-16 h-16 animate-bounce" strokeWidth={.8}/>
						</div>
						<h2 className="text-3xl font-semibold">{"Félicitation!"}</h2>
			...
			<Card className="p-4 md:p-6 bg-muted hidden">
				...
				<ul className="space-y-1 text-sm text-muted-foreground">
					<li>
						{
							"• Cliquez et faites glisser sur les lettres pour sélectionner des mots"
						}
					</li>
					<li>
						{"• Les mots peuvent être horizontaux, verticaux ou en diagonale"}
					</li>
